...
const Tabs = ({ tabs, onAddTab, onRemoveTab, onSelectTab, currentTab }) => {
  ...
  return (
    <div className="relative flex flex-row h-9 max-h-9 bg-slate-200 dark:bg-slate-800 text-sm font-light text-slate-800 dark:text-slate-200">
      
      {/* Left side draggable space */}
      <div className="max-w-20 min-w-20 draggable-element"></div>
      
      {/* Tabs container */}
      <div className="w-full overflow-x-scroll relative flex flex-row h-9 max-h-9 bg-slate-200 dark:bg-slate-800 text-sm font-light text-slate-800 dark:text-slate-200 remove-scrollbar">
        {/* Add tab button */}
        <button
          onClick={onAddTab}
          key={-1}
          className={
            'min-w-9 text-3xl font-extralight max-h-9 sticky left-0 border-t border-x-0 ' + activeStyle
          }
        >
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={
              ...
              ' min-w-32 px-2 text-nowrap overflow-clip'
            }
            style={{
              pointerEvents: activeTab === tab.id ? 'auto' : 'none',  // Deshabilitar interacción si está oculto
            }}
          >
            {tab.title}
          </button>
