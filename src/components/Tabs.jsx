...
const Tabs = ({ tabs, onAddTab, onRemoveTab, onSelectTab, currentTab }) => {
  ...
  return (
    <div
      className="flex flex-row h-9 max-h-9 bg-slate-200 dark:bg-slate-800 text-sm font-light text-slate-800 dark:text-slate-200"
    >
      <div className={'max-w-20 min-w-20 draggable-element'}>
      </div>
      <div 
        className=" w-full overflow-x-scroll draggable-element flex flex-row h-9 max-h-9 bg-slate-200 dark:bg-slate-800 text-sm font-light text-slate-800 dark:text-slate-200"
      >
        {
          [
            (
